...
| 32 | `C4` | 通用 | 六级音频决策与运行时诊断契约 | **已完成代码实施，待 guard 收尾**：只统一 `DecisionLevel`/`RuntimeState`/失败原因映射，不改变 Exo/MPV 选轨、解码或 AudioTrack 回退行为；聚焦 JVM 单测与 Mobile Arm64 Java 编译通过 | [C4-common-audio-policy-contract.md](C4-common-audio-policy-contract.md) |
