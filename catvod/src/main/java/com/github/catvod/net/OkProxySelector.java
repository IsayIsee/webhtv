    ...
    public List<java.net.Proxy> select(URI uri) {
        ...
        for (Proxy item : proxy) {
            for (String rule : item.getHosts()) {
                ...
                List<java.net.Proxy> candidates = item.getProxies();
                if (candidates.isEmpty()) return fallback(uri, "empty-proxy");
                for (java.net.Proxy p : candidates) {
                    ...
                    if (ProxyHealthChecker.get().isHealthy(addr.getHostString(), addr.getPort())) {
                        logSelection(uri, "hit", host, rule, item.getName(), 1);
                        return List.of(p);
                    }
                    if (SpiderDebug.isEnabled()) {
                        DebugEventLimiter.Decision d = acquire("skip", "unhealthy", uri, null);
                        if (d.allowed())
                            SpiderDebug.log("proxy", "select skip unhealthy uri=%s proxy=%s suppressed=%s",
                                    OkHttpLogPolicy.redactUri(uri), p, d.suppressedCount());
                    }
                }
